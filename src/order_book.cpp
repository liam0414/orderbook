...
OrderBook::OrderBook() noexcept
    : next_order_id_(1U), next_trade_id_(1U), total_trades_(0U), total_volume_(0U) {}
...
std::uint64_t OrderBook::add_order(double price, std::uint64_t quantity, Side side, OrderType type) {
    ...
    if (order->get_remaining_qty() > 0U && type == OrderType::LIMIT && 
        order->get_status() != OrderStatus::CANCELLED) {
...
std::vector<Trade> OrderBook::match_order(std::shared_ptr<Order> incoming_order) {
    ...
    if (incoming_order->get_side() == Side::BUY) {
        ...
        while (!asks_.empty() && incoming_order->get_remaining_qty() > 0U) {
            ...
            if (incoming_order->get_type() == OrderType::MARKET || 
                incoming_order->get_price() >= asks_.begin()->first) {
        ...
        while (!bids_.empty() && incoming_order->get_remaining_qty() > 0U) {
            ...
            if (incoming_order->get_type() == OrderType::MARKET || 
                incoming_order->get_price() <= bids_.begin()->first) {
...
void OrderBook::process_matches_at_level(std::shared_ptr<Order> incoming_order, 
    ...
    while (!level.empty() && incoming_order->get_remaining_qty() > 0U) {
        ...
        std::uint64_t trade_qty = std::min(
            incoming_order->get_remaining_qty(),
            resting_order->get_remaining_qty()
        );
    ...
    for (std::size_t i = 0U; i < levels && (bid_it != bids_.end() || ask_it != asks_.end()); ++i) {
        ...
        if (bid_it != bids_.end()) {
            std::cout << std::fixed << std::setprecision(2) 
                      << bid_it->second.get_total_quantity() << "@" << bid_it->first;
            ++bid_it;
        } else {
        ...
        if (ask_it != asks_.end()) {
            std::cout << std::fixed << std::setprecision(2)
                      << ask_it->second.get_total_quantity() << "@" << ask_it->first;
            ++ask_it;
        }
